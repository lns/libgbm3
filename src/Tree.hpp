...
public:
	...
	int parse(const char * text, int hint_id = -1);
	// print a node
...
public:
	...
	template<typename T>
	double predict(const std::unordered_map<T, float>& sample,
			int node_id = 0) const;
	// [Recursive] print a tree (or subtree of node_id)
	void print(int node_id = 0, FILE * fo = stdout) const;
	// Print debug info
	void dbginfo() const;
...
public:
	...
	// make raw prediction
	template<typename T>
	double predict(const std::unordered_map<T, float>& sample) const;
	// print a forest
	void print(FILE * fo = stdout) const;
};
